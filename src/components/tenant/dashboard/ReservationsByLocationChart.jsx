...
const ReservationsByLocationChart = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await tenantApi.get(
        `/${tenantId}/reservationsPerLocation`
      );
  ...
  return (
    <div className="w-full">
      <div className="pb-2 text-black/70 border-b-2 space-y-2">
        <h2 className="text-center font-semibold">Reservations by Location</h2>
        <p className="font-semibold text-left text-sm text-muted-foreground">
          No. of Reservations
        </p>
      </div>
